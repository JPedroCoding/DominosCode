...
function calculateTotal() {
    ...
    if (orderData.flavors && orderData.flavors.length > 0) {
        ...
        const avgPrice =
            orderData.flavors.reduce((sum, f) => sum + f.price, 0)
            / orderData.flavors.length;
...
function confirmOrder() {
    ...
    // MOSTRAR LOADING
    showLoading("Confirmando seu pedido...");
...
function logout() {
    ...
    if (
        confirm(
            'Deseja sair? Seu pedido em andamento será perdido.'
        )
    ) {
...
function showLoading(message = "Processando pedido...") {
